...
const USER_AGENT = 'Dezena14/Minecraft-Mods-Updater (github.com/Dezena14/Minecraft-Mods)';
const API_DELAY = 100;
...
async function getRecentMinecraftVersions() {
    try {
        ...
        return data.versions.filter(v => v.type === 'release').map(v => v.id).slice(0, 5);
    } catch (e) {
        console.error('Nao foi possivel buscar as versoes do Minecraft. Usando padrao.');
        return ['1.21', '1.20'];
    }
}
...
async function updateExistingMods() {
  ...
  const gameVersionsToCheck = versionsToCheckInput.split(',').map(v => v.trim()).filter(Boolean);

  if (gameVersionsToCheck.length === 0) {
    console.log('Nenhuma versao fornecida. Operacao cancelada.');
    return;
  }
  console.log(`OK! Verificando as versoes: ${gameVersionsToCheck.join(', ')}`);
  ...
  for (const mod of mods) {
    ...
    const apiUrl = `https://api.modrinth.com/v2/project/${mod.slug}/version?loaders=["fabric"]&game_versions=${JSON.stringify(gameVersionsToCheck)}`;
    try {
      ...
      versionsData.forEach(v => {
        v.game_versions
          // FILTRO 1: Ignora snapshots e pre-releases
          .filter(gv => /^\d+\.\d+(\.\d+)?$/.test(gv))
          // 👇 FILTRO 2 (A CORREÇÃO): Garante que a versão encontrada corresponde ao que foi pedido
          .filter(gv => gameVersionsToCheck.some(inputVersion => gv.startsWith(inputVersion)))
      ...
      console.error(` ❌ Erro ao buscar dados para ${mod.slug}: ${e.message}`);
    }
  ...
  for (const newVersion of sortedNewVersions) {
    ...
    for (const mod of mods) {
      ...
      if (modApiVersions.includes(newVersion) && !mod.version_tracker.includes(newVersion)) {
        const addInput = readline.question(`  O mod '${mod.slug}' suporta a versao ${newVersion}. Adicionar? (S/n): `);
...
async function addNewMod() {
  console.log('\n--- ADICIONAR NOVO MOD ---');
  const slug = readline.question('Digite o "slug" do mod a ser adicionado (ex: sodium): ');
  if (!slug) {
  ...
  try {
    ...
    const projectRes = await fetch(projectUrl, { headers: { 'User-Agent': USER_AGENT } });
    if (!projectRes.ok) throw new Error(`Mod nao encontrado (status ${projectRes.status})`);
    
    const projectData = await projectRes.json();
    const confirmAdd = readline.question(`? Mod encontrado: '${projectData.title}'. Adicionar a curadoria? (S/n): `);
    ...
    if (confirmAdd.toLowerCase() === 's') {
      ...
      if (gameVersionsToCheck.length > 0) {
        ...
        const versionsUrl = `https://api.modrinth.com/v2/project/${slug}/version?loaders=["fabric"]&game_versions=${JSON.stringify(gameVersionsToCheck)}`;
        ...
        if (supportedVersions.size === 0) {
          console.log(`Nenhuma versao compativel encontrada para '${slug}' nos filtros definidos.`);
        } else {
          console.log('--- Curadoria Inicial de Versoes ---');
          Array.from(supportedVersions).sort(naturalSort).forEach(v => {
            const addVersion = readline.question(`  '${slug}' suporta a versao ${v}. Adicionar? (s/n): `);
